...
=============
  ...
  FIX 2 — Table slab clips base_link_inertia, AND elbow can dip under it:
      ...
      TABLE_Z - 0.25 m. Top face still at -0.01 m, bottom at -0.51 m.
      Two problems solved:
  ...
  Speed change:
      VELOCITY_SCALE / ACCELERATION_SCALE = 0.03 (3 % of rated speed)
      for extra caution during initial tuning.
"""
...
# FIX 2: top face at -0.01 m, slab now 50 cm thick to act as no-go floor
TABLE_Z = -0.01
...
def setup_collision_scene(arm: MoveIt2) -> None:
    """
    Table slab as no-go floor (no ceiling — UR3e links exceed 0.35 m
    in normal configs).

    FIX 2: 50 cm thick slab. Top face at TABLE_Z = -0.01 m, bottom at
    -0.51 m. Two effects:
    ...
    arm.add_collision_box(
        id="table",
        size=[2.0, 2.0, 0.50],                      # 50 cm thick floor
        frame_id=BASE_LINK,
