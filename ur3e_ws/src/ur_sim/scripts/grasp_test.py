...
GRASP_SAMPLES        = 10     # collect this many before computing median
GRASP_TIMEOUT_SEC    = 30.0   # total time to gather GRASP_SAMPLES
...
def wait_for_grasp(node: Node):
    ...
    if n < GRASP_SAMPLES:
        print(f"   ⚠️   Got only {n}/{GRASP_SAMPLES} samples — proceeding anyway")
    ...
    print(f"   📊  Spread   : ΔX={sx*1000:.1f} mm  ΔY={sy*1000:.1f} mm")
    if sx > 0.02 or sy > 0.02:
        print(f"   ⚠️   Spread > 2 cm — detection is noisy; check lighting / shoe pose")
...
def validate_grasp(x: float, y: float, z: float) -> bool:
    ...
    if not (PICK_X_MIN <= x <= PICK_X_MAX):
        print(f"   ❌  Grasp X={x:+.3f} outside [{PICK_X_MIN}, {PICK_X_MAX}]")
        ok = False
    if not (PICK_Y_MIN <= y <= PICK_Y_MAX):
        print(f"   ❌  Grasp Y={y:+.3f} outside [{PICK_Y_MIN}, {PICK_Y_MAX}]")
        ok = False
    if z < TABLE_Z - 0.05 or z > 0.30:
        print(f"   ⚠️   Grasp Z={z:+.3f} unusual (table at {TABLE_Z:.2f}) — "
